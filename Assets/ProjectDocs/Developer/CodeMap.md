...
| Actions / Planning | [Battle/Actions](../../Scripts/Battle/Actions) | BattleActionSlotManager | ActionSlot、placement、response；`TryFindCurrentResponseSlot` 提供最终 response pair 只读查询 | UI Router/Controller | Card eligibility、Targeting | ActionSlot/status UI、paired card detail | assignment/interaction Legacy | BattleScene | CURRENT / 高；DEFERRED_DEBT |
...
| UI | [UI](../../Scripts/UI) | 局部 View/Host；Core/BattleSimpleUIController 跨流程协调 | RuntimeState、CardState、Prefab 字段；Action Slot Card Info normal/paired display state | BattleSimpleUIController → BattleCharacterStatusUIView → BattleActionSlotUIView；`BattleActionSlotCardInfoPanelHost`；Scene/UI input | Planning/Lifecycle/Spawner/Presentation；BattleActionSlotManager final response query | Hand/Card/Slot/详情/Roll Prefab | UI/关键词/关系线 Legacy；Action Order View / Paired Hover Suite | BattleScene/Preview | CURRENT / 高；Controller DEFERRED_DEBT |
...
- UI paired response detail coverage：`BattleActionSlotPairedHoverTests` 由 CardLoadTest Mode105 retained runner 调用，覆盖最终 responder lookup 与双向 request payload；paired cleanup / locked coexistence 仍需 Unity manual verification。
...
- Mode78 `BattleInteractionStateAndEndLockTests` 保留 Planning 交互与默认来源 Slot1 选择回归；默认来源 owner 是 [Core/BattleSimpleUIController.cs](../../Scripts/Core/BattleSimpleUIController.cs)。
