...
## 九十二、行动槽位卡牌详情改为点击锁定并显示实际卡面

### 一、点击锁定规则

`BattleActionSlotCardInfoPanel` 已由悬停触发改为点击触发。

当前规则：

- 左键点击有卡牌的行动槽位后立即显示对应卡牌详情。
- 光标离开槽位后不关闭详情。
- 点击另一个有卡牌的槽位后切换为新槽位的卡牌。
- 点击空槽位不会覆盖当前已经锁定的卡牌详情。
- 已锁定槽位的运行时卡牌引用变化时，详情会同步刷新。
- 已锁定槽位被清空、禁用或重新绑定时，详情会安全关闭，避免保留失效引用。
- 我方槽位继续使用左上角面板，敌方槽位继续使用右上角面板。

为保持现有接线和避免无关重命名，宿主入口仍保留：

```csharp
BattleActionSlotCardInfoPanelHost.HandlePointer(...)
```

但该入口当前承载的是槽位点击锁定与来源失效通知，不再由 Pointer Enter / Exit 控制显示周期。

### 二、左右面板显示实际行动卡牌

`BattleActionSlotCardInfoPanel.prefab` 的左右 View 现在都直接引用：

```text
Assets/Art/battle/kapai/BattleCardUI.prefab
```

运行时会在原 `CardArtwork` 区域生成只读卡面预览，并通过：

```csharp
BattleCardUIPreviewBuilder.Build(...)
BattleCardUIView.BindCard(...)
```

显示当前槽位对应卡牌的名称、类型、品质框、点数和说明。

预览卡规则：

- 按 `CardArtwork` 的实际宽高等比缩放并居中。
- 关闭悬浮位移与选中动画。
- 关闭独立射线接收，不影响行动槽位点击。
- 左右两侧使用同一张正式卡牌 UI 预设体，不复制第二套卡面实现。

原有详情文字仍保留，包括角色、点数、类型、基础 CD、说明、当前状态、使用次数和关键词。

### 三、预设体生成工具同步

`BattleSecondaryInfoPrefabGenerator` 已同步新的卡面预览引用和校验，并新增只重建目标预设体的菜单：

```text
Project Guilt/UI/重新生成行动槽位卡牌详情预设体
```

原“重新生成二级信息面板预设体”菜单仍然保留。
...
- `BattleActionSlotCardInfoPanel.prefab` 的左右两个 View 均已写入有效 `BattleCardUI` 预设体引用。
- `git diff --check` 通过。
- 当前项目由交互式 Unity 编辑器占用，因此没有强制关闭编辑器执行独立批处理 Play Mode；最终点击手感和卡面尺寸仍应在当前 BattleScene 中进行一次人工确认。

## 九十三、友方与敌方行动槽位卡牌详情支持同时显示

### 一、双侧独立锁定

`BattleActionSlotCardInfoPanelHost` 已将原来全局唯一的选中状态拆分为：
...
- 点击友方有卡槽位，只更新左上角友方面板。
- 点击敌方有卡槽位，只更新右上角敌方面板。
- 友方面板显示后，再点击敌方槽位不会关闭友方面板。
- 敌方面板显示后，再点击友方槽位不会关闭敌方面板。
- 左右两个二级面板可以同时显示各自行动槽对应的行动卡牌。
- 同一阵营点击其他有卡槽位时，只切换该阵营自己的面板内容。
...
- 光标移出不关闭。
- 点击空槽位不覆盖当前已经锁定的详情。
- 已锁定槽位卡牌引用变化时同步刷新对应一侧。
- 已锁定槽位被清空、禁用或重新绑定时，只关闭对应一侧。
- 友方固定显示在左上角，敌方固定显示在右上角。
- 左右两侧继续复用正式 `BattleCardUI.prefab` 卡面。
- 原有安全区域、分辨率适配和最前层 Canvas 规则不变。

### 三、改动边界与验证

本次只修改二级面板宿主的双侧选中状态，没有修改：

- `BattleActionSlotUIView` 的槽位点击与绑定规则。
- `BattleActionSlotCardInfoPanel.prefab` 的布局和引用。
- 行动安排、战斗结算或敌人意图。

验证记录：

- `ProjectGuilt.sln` 完整编译通过，0 警告、0 错误。
- 静态检查确认显示一侧时不再调用 `HidePanels()` 关闭另一侧。
- `git diff --check` 通过。

## 九十四、行动槽位二级面板精简为可锁定放大卡面

### 一、面板内容精简

`BattleActionSlotCardInfoPanel` 已移除原有角色、点数、类型、CD、状态和关键词列表等独立详情列。

当前每侧面板只保留：

- 复用正式 `BattleCardUI.prefab` 的放大卡面。
- 一个手动关闭按钮。

放大卡面继续使用 `BattleCardUIView` 绑定运行时卡牌数据，并恢复卡面射线接收，因此卡牌描述中的关键词可以正常触发通用二级信息面板。

### 二、行动槽位输入规则

当前规则：

- 光标进入有卡行动槽时，临时显示该槽位的放大卡面。
- 光标移出且该槽位未点击锁定时，关闭临时卡面。
- 左键点击有卡行动槽时，锁定该侧卡面；移出槽位后继续显示。
- 悬停同一侧其他有卡槽位时，临时预览新卡牌；移出后恢复已锁定卡牌。
- 点击关闭按钮时，只关闭并清除对应阵营一侧的卡面。
- 友方与敌方的悬停、锁定和关闭状态继续相互独立。
- 空槽位不会覆盖已经锁定的卡面。
- 槽位卡牌被清空、禁用或重新绑定时，会清理对应失效请求。
...
- `Assets/Prefabs/BattleActionSlotCardInfoPanel.prefab`。
- `BattleSecondaryInfoPrefabGenerator` 的生成和校验规则。
- `BattleScene` 中的行动槽卡牌面板预设体实例。

预设体生成器会识别仍包含旧 `InfoColumn` 或缺少关闭按钮的旧版资源，并只升级行动槽卡牌面板；显式重建时也会替换场景中的旧实例，避免遗留失效 Prefab Override。
...
- `ProjectGuilt.sln` 完整编译通过，0 警告、0 错误。
- Unity 6000.3.7f1 成功生成新版行动槽卡牌面板预设体。
- 预设体确认包含两个卡面 View、两个关闭按钮和两个正式卡牌预设体引用。
- 预设体中旧 `InfoColumn` 数量为 0。
- `BattleScene` 中行动槽面板的 Prefab Override 均可解析，无失效 fileID。
- Unity 预设体与 `BattleScene` 实例校验通过。

## 九十五、修复目标箭头区域的行动槽卡面闪烁

### 一、问题原因

行动槽放大卡面为了支持关键词交互会接收 UI 射线。当放大卡面覆盖到行动槽目标箭头附近的光标位置时，行动槽会立即收到移出事件并关闭卡面；卡面关闭后光标又重新命中行动槽，形成反复显示与关闭。

关系线和箭头本身继续保持不接收射线，本次没有修改关系显示规则。

### 二、修复规则

- 行动槽移出不再立即清除临时卡面，而是在本帧输入事件处理完成后统一确认。
- 光标从行动槽进入放大卡面时，由卡面接续悬停状态，面板保持显示。
- 光标同时离开行动槽和放大卡面后，未锁定的临时卡面正常关闭。
- 点击锁定、手动关闭和双侧独立显示规则保持不变。

### 三、改动边界与验证

- 只修改行动槽卡面宿主和卡面 View 的指针状态衔接。
- 预设体已有对应 View 组件，不需要新增序列化字段或改动布局。
- `ProjectGuilt.sln` 完整编译通过，0 警告、0 错误。

## 九十六、技能面板缩放与战斗空白区右键取消

### 一、技能面板尺寸

行动槽位技能面板的运行时高度比例由屏幕高度的 `72%` 调整为 `50%`，高度限制调整为 `260 - 560`，继续按照正式卡牌的 `780:1150` 比例等比缩放。

在 1920×1080 参考分辨率下，面板尺寸约为：

```text
366 × 540
```

左右两侧尺寸、安全区域边距和关键词交互规则保持一致。

### 二、战斗空白区右键取消

战斗场景中按下鼠标右键时，会通过 `EventSystem` 检查当前指针位置：

- 空白区域右键：关闭友方与敌方技能面板，并取消当前角色行动槽选择、卡牌选择、角色卡牌栏和临时目标箭头。
- 卡牌、行动槽、按钮、拖拽区或滚动区上右键：不触发全局关闭，保留对应 UI 自身的输入逻辑。
- 该操作只清理规划期临时 UI，不取消已经写入 Runtime 的正式行动安排。
- 友方行动槽上的原有右键取消行动安排规则保持不变。

### 三、敌方目标槽位点击

选定我方行动槽与卡牌后，点击敌方行动槽属于目标确认输入：

- 本次敌方槽位点击不会再锁定敌方技能面板。
- 行动安排成功后，自动关闭当前友方与敌方技能面板。
- 行动安排成功后，继续使用既有统一清理入口关闭角色卡牌栏和规划选择。
- 没有选择卡牌时，普通点击敌方行动槽仍可以锁定查看敌方技能面板。

### 四、预设体同步

已同步更新：

- `Assets/Prefabs/BattleActionSlotCardInfoPanel.prefab` 左右面板尺寸。
- `BattleSecondaryInfoPrefabGenerator` 的生成尺寸。

`BattleScene` 中的实例没有左右面板尺寸 Override，会直接继承新版预设体。

验证记录：

- `ProjectGuilt.sln` 完整编译通过，0 警告、0 错误。
- 当前 Unity 编辑器完成程序集重载，日志没有 C# 编译错误。
- 预设体左右面板均为 `366×540`，卡牌和关闭按钮引用保持完整。
- `BattleScene` 继续引用新版预设体 GUID，左右面板尺寸 Override 数量为 0。
- `git diff --check` 通过。
...
- 空行动槽：关闭当前角色卡牌栏，并清除行动槽选择、卡牌选择和临时目标箭头。
- 已安排行动的槽位：继续执行原有的右键取消行动安排规则。

空槽右键只清理规划期临时 UI，不修改其他槽位已经写入 Runtime 的正式行动安排。该输入继续复用行动槽现有右键回调，不需要新增或修改预设体接线。
